...
function prox(::KL{ρ}, ϵ, x) where {ρ}
    f(ξ) = ξ + (ε/ρ)*exp(ξ) - x
    ξ0 = find_zero(f, (-100*(ρ/ε+abs(x)), 100*(ρ/ε+abs(x))))
    q0 = ξ0 * ρ
    return q0 # (x - q0)^2/(2ϵ) + ((x - q0)/ϵ - 1) ρ
end
...
function prox(::RG{a,b}, ϵ, x) where {a,b}
    if (s = x - ϵ * a) < 0
        s
    elseif (t = x - ϵ * b) > 0
        t
...
function prox(::TV{ρ}, ϵ, x) where {ρ}
    if -ρ <= x <= ρ
        x
    elseif x < -ρ
        -ρ
